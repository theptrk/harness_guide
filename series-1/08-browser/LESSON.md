...
```sh
uv run --env-file .env series-1/08-browser/main.py --new
```
...
`open_page` returns a JSON snapshot of the rendered Google page. The relevant
interactive element should be a search field with attributes similar to:

```text
{"tag": "textarea", "name": "q", "role": "combobox", "aria_label": "Search"}
```

Those attributes let the model construct the CSS selector
`textarea[name="q"]`. It fills that field and presses Enter in one tool call:

```text
tool › type_text({
  "selector":"textarea[name=\"q\"]",
  "text":"On what date did the Golden Gate Bridge open to vehicle traffic?",
  "press_enter":true
})
```

Google may return search results directly. In our automated test it returned:
...
The tool returns that text to the model. `main.py` tells the model how to handle
that condition:
...
The model stops and asks for help:

```text
🤖 model › Google is asking for human verification. Complete it in the open
browser, then tell me when you are done.
```

Keep the Level 8 process and Chromium window open. Complete the verification in
Chromium, return to the terminal, and enter:

```text
📝 you › I completed the verification. Continue.
```

The shared Playwright `Page` still represents that browser tab. The model calls
`read_page()` to inspect it, uses `click()` to open a credible result, and
answers from the rendered source. If Google does not show a verification page,
the model continues without asking for help.
...
The agent loop is unchanged. If the model returns a `function_call` named
`click`, `run_tool()` selects `browser_tools.click` from this dictionary and
...
why the number created by `click` is still present when `read_page` runs in a
later user turn.
...
2. Run `uv run --env-file .env series-1/08-browser/main.py --new`.
3. Enter the Google request under **Run it**.
4. Confirm `open_page` opens Google and reports a search field.
5. Confirm `type_text` fills the field and submits the query with
   `"press_enter": true`.
6. If Google shows search results, continue to step 9.
7. If Google shows human verification, confirm the model asks you to complete
   it instead of claiming the search succeeded.
8. Keep the process open, complete the verification in Chromium, then enter
   `I completed the verification. Continue.`
9. Confirm the model calls `read_page`, opens a credible result with `click`,
   and reports the date and source URL.
