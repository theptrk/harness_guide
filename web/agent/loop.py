"""Run one user turn: model calls, tools, and history. No terminal I/O."""
...
from . import browser_tools, file_tools, history, shell_tools, workspace
...
def tool_functions(approve: Approve) -> dict:
    """Bind tool names to Python functions for one turn."""
    return {
        ...
        "run_command": lambda command: shell_tools.run_command(command, approve=approve),
...
def run_tool(tool_call, functions: dict) -> str:
    """Run one requested tool, converting any failure into tool output."""
    try:
        arguments = json.loads(tool_call.arguments)
        tool_function = functions.get(tool_call.name)
...
def run_turn(
    ...
    *,
    ...
    turn_id = uuid4().hex
    functions = tool_functions(approve)
    ...
    try:
        while True:
            ...
            if tool_calls >= TOOL_CALL_LIMIT:
                ...
                tool_result = run_tool(tool_call, functions)
                tool_calls += 1
...
def transcript_entries(path) -> list[dict]:
    """Turn completed history events into UI rows."""
    if not path.exists() or path.stat().st_size == 0:
        return []
    entries = []
    for event in history.read_events(path):
        if event.get("kind") != "api_item":
            continue
        item = event["item"]
        at = event.get("at")
        if item.get("role") == "user" and item.get("type") in {None, "message"}:
            text = item_text(item)
            if text:
                entries.append({"kind": "user", "text": text, "at": at})
            ...
            text = item_text(item)
            if text:
                entries.append({"kind": "assistant", "text": text, "at": at})
    return entries
...
def handle_message(said: str, *, emit: Emit, approve: Approve) -> None:
    """Run one user message. The host supplies emit and approve."""
    try:
...
def shutdown() -> None:
    """Release the persistent browser if it was started."""
...
def current_chat():
    """Return the latest chat file, creating one if needed."""
    return history.latest_chat() or history.new_chat()
...
def snapshot() -> dict:
    """Return the current conversation for a host UI."""
    path = current_chat()
    return {
        "title": conversation_title(path),
        "workspace": workspace_label(),
