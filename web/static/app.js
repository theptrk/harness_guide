...
function appendCard(kind, text, iso) {
  appendTime(iso);
  const row = document.createElement("div");
...
async function loadConversation() {
  ...
  for (const message of data.messages) {
    if (message.kind === "user") {
      appendMessage("user", message.text, message.at);
    } else if (message.kind === "assistant") {
      appendMessage("assistant", message.text, message.at);
    } else if (message.kind === "tool") {
      appendCard("tool", `${message.name}(${message.arguments})`, message.at);
    } else if (message.kind === "tool_result") {
      appendCard("tool-result", formatToolResult(message.output), message.at);
    }
  }
}
...
approveYes.addEventListener("click", () => resolveApproval(true));
approveNo.addEventListener("click", () => resolveApproval(false));
