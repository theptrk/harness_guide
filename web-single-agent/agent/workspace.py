...
ROOT = Path(__file__).resolve().parent.parent / "agent_workspace"
READ_LINE_LIMIT = 200
...
def read_file(path: str, offset: int = 1, limit: int = READ_LINE_LIMIT) -> dict:
    """Return a line window from one UTF-8 text file."""
    if offset < 1:
    ...
    start = offset - 1
    window = lines[start : start + min(limit, READ_LINE_LIMIT)]
    numbered = [f"{start + index + 1}|{line}" for index, line in enumerate(window)]
    return {
        "path": path,
        "offset": offset,
        "lines": numbered,
        "total_lines": total_lines,
        "truncated": start + len(window) < total_lines,
    }
